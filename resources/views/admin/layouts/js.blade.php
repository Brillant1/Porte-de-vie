<script src="{{ asset("admin/js/jquery.min.js") }}"></script>
<script src="{{ asset("admin/js/global.min.js") }}"></script>
<script src="{{ asset("admin/js/custom.min.js") }}"></script>
<script src="{{ asset("admin/js/dashboard-2.js") }}"></script>
<script src="{{ asset("admin/js/vivus.min.js") }}"></script>
<script src="{{ asset("admin/js/svg.animation.js") }}"></script>
<script src="{{ asset("admin/js/bootstrap.min.js") }}"></script>
<script src="{{ asset("admin/js/jquery.dataTables.min.js") }}"></script>
<script src="{{ asset("admin/js/datatables.init.js") }}"></script>
<script src="{{ asset("admin/js/adminScript.js") }}"></script>
<script src="{{ asset("admin/js/lightbox-plus-jquery.js") }}"></script>
<script src="{{ asset('admin/js/yearpicker.js') }}"></script>
<script src="{{ asset('admin/ckeditor5/ckeditor.js') }}"></script>
<script src="{{ asset('admin/js/jquery.magnific-popup.js') }}"></script>
<script src="{{ asset('admin/js/jquery.dataTables.min.js') }}"></script>
<script src="{{ asset('admin/css/bootstrap-select.min.css') }}"></script>
<script src="{{ asset('admin/js/dlabnav-init.js') }}"></script>



{{-- <script src="https://cdn.ckeditor.com/ckeditor5/34.2.0/classic/ckeditor.js"></script> --}}



